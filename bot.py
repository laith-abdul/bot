...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7071855340:AAF2auNib1FGZD6LL8zaoROECa5S1uo_-Jo")
if not BOT_TOKEN:
    raise RuntimeError(
...
# الحد الأقصى لمدة المقطع عند التحميل من رابط (بالثواني)
MAX_URL_DURATION_SECONDS = 20 * 60  # 20 دقيقة
...
def _ydl_proxy_opts() -> dict:
    """
    خيارات البروكسي المشتركة لكل نداءات yt-dlp. نستخدم نفس بروكسي تلجرام
    (TELEGRAM_PROXY_URL) لأن نفس تدخّل الشبكة قد يمنع التحميل من الروابط
    (يوتيوب/تيك توك/إلخ) تماماً مثلما يمنع الرفع لتلجرام
    """
    return {"proxy": TELEGRAM_PROXY_URL} if TELEGRAM_PROXY_URL else {}
...
def run_demucs(input_path: Path, job_dir: Path, model: str, two_stems: Optional[str], progress_cb=None) -> dict:
    ...
    if process.returncode != 0:
        raise RuntimeError("فشلت معالجة Demucs: " + " | ".join(tail_lines[-5:]))
...
async def receive_sep_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(t(lang, "btn_fast"), callback_data="quality_fast")],
